...
public class ReviewServiceTest {
    ...
    void userCannotCreateMoreThanOneReviewPerPos() {
        // given
        Review originalReview = TestFixtures.getReviewFixtures().getFirst();
        //had to change this test, otherwise it tries to update a review in upsert, instead of creating a new one
        Review review = originalReview.toBuilder()
                .id(null)
                .build();
        Pos pos = review.pos();
        ...
        when(posDataService.getById(pos.getId())).thenReturn(pos);
        when(reviewDataService.filter(pos, author)).thenReturn(List.of(originalReview));
